...
- 成对符号智能输入受 `TypingConfig.pair_matching` 控制；关闭时必须按原 token 顺序线性输出，不生成光标移动动作。
- 开启时仅对 50 个原文字符内已匹配的非空成对符号移动光标；空对、未匹配或交叉错配时，当前符号按普通字符处理，后续 token 继续按当前规则规划。
- 孤儿闭符号按普通字符处理。
...
- 配置项或预设新增时同步更新首页状态、配置页交互和提交后的命令行确认信息。
