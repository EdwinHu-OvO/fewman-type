# 类人输入 (FewmanType)

**“离人很远了，但还在努力装得像个人。”**

**FewmanType**（发音形似 Human Type，意为“很少有人”，暗示离人很远）是一个基于 Rust 开发的整活项目。它的主要功能是把你粘贴进去的大段文本，通过“模拟真实人类敲击键盘”的方式，一字一句地自动输入到目标窗口中。

无论是为了应付某些检测粘贴的系统，还是单纯为了在朋友面前装作自己手速惊人，它都能派上用场。

## ✨ 核心特性

- **全局热键拦截**：复制好文本后，切到目标窗口按下 `Ctrl+V`，程序会拦截粘贴操作，并瞬间化身键盘侠开始自动打字。
- **类人延迟机制**：支持基础输入间隔配置，每个字符之间的敲击延迟会有随机波动，告别机械式的匀速输入。
- **智能中文拆词**：支持加载外部词库（如 `rime_ice_words.yaml`）进行 CJK 拆词。程序会把词组当成一个整体输入，词内打字速度极快，词间会有自然停顿，完美还原人类真实打字节奏。
- **随时紧急刹车**：如果打错地方了？别慌，随时按下 `ESC` 键即可紧急停止输入。
- **终端图形界面 (TUI)**：简单易用的命令行图形界面，方便配置参数和输入要打字的文本。

## 🚀 待实现功能 (TODO)

- [ ] **基于拼音和键盘位置的真实输入错误**：人非圣贤孰能无过！未来将支持基于键盘物理键位（例如按错相邻按键）和拼音相似度模拟人类打字时的手误和修改过程，让“类人”程度更上一层楼。

## 🛠️ 安装与使用

### 准备工作

请确保你的电脑上安装了 Rust 环境（[Rust 官网](https://www.rust-lang.org/)）。

### 编译运行

1. 克隆本仓库：
   ```bash
   git clone https://github.com/EdwinHu-OvO/fewman-type-rs.git
   cd fewman-type-rs
   ```

2. 运行程序：
   ```bash
   cargo run --release
   ```
   *(推荐使用 `--release` 构建，以获得更稳定的性能)*

### 使用步骤

1. 打开程序后，在 TUI 界面中输入或粘贴你想要“打”出来的文本。
2. 配置好**输入间隔**、**是否开启中文拆词**等参数，然后确认。
3. 将鼠标光标点击到你想要输入的任何地方（比如微信聊天框、Word 文档、代码编辑器或某个网页中）。
4. 按下 **`Ctrl+V`**，奇迹开始！
5. 如果想中途退出，按下 **`ESC`** 键即可。

## 📝 配置文件与词库

如果需要更强大的中文拆词体验，可以将词库文件（例如 `rime_ice_words.yaml`）放置在程序运行的当前目录下。程序启动时会自动检测并加载 `*_words.yaml` 格式的词库文件。如果未发现词库，程序将回退到内置的简易词表。

## ⚠️ 免责声明

本项目仅供学习、研究及**整活**使用。请勿用于破坏系统、作弊或其他违反相关平台规定的行为。由于过度真实的打字模拟可能导致您的朋友/同事/老师产生不必要的敬畏，作者对此不承担任何责任。

---
*Built with Rust 🦀, `enigo`, `rdev`, and `cursive`.*
